...
def run_multi_ev_v2g_simulation(cfg: dict):
    """Main entry point – runs the whole multi-EV V2G experiment."""
    duration = cfg['duration']

    # ------------------------------------------------------------------ #
    # 1. Convert list → dict for grid capacity and sell prices
    # ------------------------------------------------------------------ #
    ...
    if isinstance(cfg.get('sell_price_profile'), list):
        cfg['sell_price_profile'] = {
            h: cfg['sell_price_profile'][h] for h in range(24)
        }
    ...
    # ------------------------------------------------------------------ #
    # 4. V2G Charging system
    # ------------------------------------------------------------------ #
    system = MultiEVV2GChargingSystem(
        building=building,
        evs=evs,
        grid=grid,
    ...
    methods = {
        ...
        'rl': lambda h: system.rl_charge_multi(
            h,
            episodes=cfg.get('rl_episodes', 2000),
    ...
    # End simulation when the last EV leaves (max target_time)
...
def visualise_multi_ev_v2g(results: dict, cfg: dict, evs: list, system):
    """Visualize multi-EV V2G simulation results."""
    hours = results['hours']
    ...
    ax1.set_title('EV Fleet State-of-Charge (V2G Enabled)', fontsize=14, fontweight='bold')
    ...
    ax3.set_ylabel('Hourly Benefit (€)', fontsize=12)
    ax3.set_title('Hourly Financial Benefit (Positive = Revenue, Negative = Cost)',
    ...
    for name, color in colors.items():
        ...
        ax4.plot(hours, cum, label=f'{name.upper()}', color=color, lw=2.5, marker='o', markersize=5)

    ax4.set_xlabel('Hour of Day', fontsize=12)
    ax4.set_ylabel('Cumulative Benefit (€)', fontsize=12)
...
def summarise_multi_ev_v2g(results: dict, cfg: dict, system):
    """Print summary statistics for multi-EV V2G simulation."""
    print("\n" + "=" * 90)
    print("MULTI-EV V2G SIMULATION SUMMARY")
    print("=" * 90)
    print(f"{'Method':<8} {'Total Benefit (€)':>18} {'Final Avg SoC':>16} "
          f"{'Grid Violations':>18} {'SoC Violations':>16}")
    print("-" * 90)

    for name in ['simple', 'rl', 'milp', 'mpc']:
        if name not in results:
            continue
    ...
    print(
        f"  - Grid capacity range: {min(cfg['grid_capacity_per_hour'].values())}-{max(cfg['grid_capacity_per_hour'].values())} kW")
    print("=" * 90)
