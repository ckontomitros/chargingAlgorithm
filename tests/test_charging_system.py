...
class TestChargingSystem(unittest.TestCase):
    def setUp(self):
        ...
        self.max_discharge_rate = 7
        self.usage_stats = {'departure': 8, 'arrival': 18, 'daily_km': 50}
        self.desired_soc = 0.8
        self.target_time = 12
        self.arrival_time = 8

    def create_charging_system(self, initial_soc=None, arrival_time=None, target_time=None, price_profile=None,
                               renewable_profile=None):
        """Helper to create ChargingSystem instance with customizable parameters."""
        ...
        grid = Grid(price_profile)
        return ChargingSystem(building, ev, grid, self.desired_soc, target_time, arrival_time)
    ...
    def test_simple_charge_multiple_hours(self):
        ...
        for hour in range(8, 12):
            ...
            self.assertTrue(should_charge)
            total_cost += cost
        self.assertGreaterEqual(charger.ev.soc, 0.766)  # Should reach desired SoC
        expected_energy = (0.8 - 0.5) * 60  # 30 kWh needed
        expected_cost = 7 * 0.15 + 7 * 0.12 + 7 * 0.12 + 7 * 0.10   # First 4 hours
    ...
    def test_cost_optimized_charge_cheapest_hour(self):
        ...
        should_charge, cost = charger.cost_optimized_charge(hour=8)  # Hour 10: 0.10 €/kWh
        self.assertTrue(should_charge)
        self.assertAlmostEqual(cost, 7 * 0.10)  # 7 kWh * 0.10 €/kWh
        self.assertAlmostEqual(charger.ev.soc, 0.5 + 7 / 60)

    def test_cost_optimized_charge_non_cheapest_hour(self):
        charger = self.create_charging_system()
        should_charge, cost = charger.cost_optimized_charge(hour=10)  # Hour 8: 0.15 €/kWh, not cheapest
        self.assertFalse(should_charge)
        self.assertEqual(cost, 0)
        self.assertEqual(charger.ev.soc, 0.5)  # SoC unchanged
    ...
    def test_cost_optimized_charge_with_renewables(self):
        ...
        should_charge, cost = charger.cost_optimized_charge(hour=8)  # Hour 8: 0.10 €/kWh, 20 kWh renewable
        self.assertTrue(should_charge)
    ...
    def test_cost_optimized_charge_short_window(self):
        ...
        should_charge, cost = charger.cost_optimized_charge(hour=11)
    ...
    def test_rl_charge_low_price_hour(self):
        charger = self.create_charging_system()
        should_charge, cost = charger.rl_charge(hour=8, episodes=100)  # Hour 8: 0.10 €/kWh, low price
        self.assertTrue(should_charge)  # RL should prefer low-price hour
        self.assertAlmostEqual(cost, 7 * 0.10, places=2)
        self.assertAlmostEqual(charger.ev.soc, 0.5 + 7 / 60)

    def test_rl_charge_soc_reached(self):
        charger = self.create_charging_system(initial_soc=0.8)
        should_charge, cost = charger.rl_charge(hour=10, episodes=100)
        self.assertFalse(should_charge)
        self.assertEqual(cost, 0)
    ...
    def test_rl_charge_with_renewables(self):
        renewable_profile = [0, 0, 20, 20, 0]  # High renewable energy at hours 2 and 3
        charger = self.create_charging_system(renewable_profile=renewable_profile)
        should_charge, cost = charger.rl_charge(hour=8, episodes=100)  # Hour 8: 0.10 €/kWh, 20 kWh renewable
        self.assertTrue(should_charge)  # RL should charge due to low effective cost
