...
class TestMultiEVChargingSystem(unittest.TestCase):
    ...
    def test_rl_no_discharge_action(self):
        """Test that RL algorithm doesn't have discharge action"""
        print("\n=== Testing RL No Discharge Action ===")

        system = MultiEVChargingSystem(self.building, self.evs, self.grid, self.grid_capacity)

        # Run RL and check no EV discharges
        for hour in range(10, 15):
            initial_socs = [ev_cfg['ev'].soc for ev_cfg in self.evs]
            system.rl_charge_multi(hour, episodes=200)
            final_socs = [ev_cfg['ev'].soc for ev_cfg in self.evs]
            
            for i in range(len(self.evs)):
                self.assertGreaterEqual(
                    final_socs[i],
                    initial_socs[i] - 0.001,
                    f"EV {i} should not discharge with RL at hour {hour}"
                )
        
        print("✓ RL no discharge test passed")
    ...
    def test_milp_no_discharge(self):
        """Test that MILP only charges, never discharges"""
        print("\n=== Testing MILP No Discharge ===")
            ...
            for hour in range(10, 15):
                ...
                for i in range(len(self.evs)):
                    self.assertGreaterEqual(
                        final_socs[i],
                        initial_socs[i] - 0.001,
                        f"MILP: EV {i} should not discharge at hour {hour}"
                    )
            
            print("✓ MILP no discharge test passed")

        except ImportError:
            print("⚠ SKIP: MILP no discharge test (PuLP not installed)")
            self.skipTest("PuLP not installed")
    ...
    def test_mpc_no_discharge(self):
        """Test that MPC only charges"""
        print("\n=== Testing MPC No Discharge ===")

        system = MultiEVChargingSystem(self.building, self.evs, self.grid, self.grid_capacity)

        for hour in range(10, 14):
            initial_socs = [ev_cfg['ev'].soc for ev_cfg in self.evs]
            system.mpc_charge(hour, horizon=3, iterations=20)
            final_socs = [ev_cfg['ev'].soc for ev_cfg in self.evs]
            ...
            for i in range(len(self.evs)):
                self.assertGreaterEqual(
                    final_socs[i],
                    initial_socs[i] - 0.001,
                    f"MPC: EV {i} should not discharge at hour {hour}"
                )
        
        print("✓ MPC no discharge test passed")
    ...
    def test_pso_no_discharge(self):
        """Test that PSO only charges, never discharges"""
        print("\n=== Testing PSO No Discharge ===")

        system = MultiEVChargingSystem(self.building, self.evs, self.grid, self.grid_capacity)

        for hour in range(10, 14):
            initial_socs = [ev_cfg['ev'].soc for ev_cfg in self.evs]
            system.pso_charge(hour, n_particles=15, n_iterations=20)
            final_socs = [ev_cfg['ev'].soc for ev_cfg in self.evs]
            ...
            for i in range(len(self.evs)):
                self.assertGreaterEqual(
                    final_socs[i],
                    initial_socs[i] - 0.001,
                    f"PSO: EV {i} should not discharge at hour {hour}"
                )
        
        print("✓ PSO no discharge test passed")
    ...
    #
    # def test_dqn_no_discharge(self):
    #     """Test that DQN only charges"""
    #     print("\n=== Testing DQN No Discharge ===")
    #
    #     try:
    #         import tensorflow
    #
    #         system = MultiEVChargingSystem(self.building, self.evs, self.grid, self.grid_capacity)
    #
    #         for hour in range(10, 12):  # Fewer hours due to training time
    #             initial_socs = [ev_cfg['ev'].soc for ev_cfg in self.evs]
    #             system.dqn_charge(hour, episodes=50, batch_size=16)
    #             final_socs = [ev_cfg['ev'].soc for ev_cfg in self.evs]
    #
    #             for i in range(len(self.evs)):
    #                 self.assertGreaterEqual(
    #                     final_socs[i],
    #                     initial_socs[i] - 0.001,
    #                     f"DQN: EV {i} should not discharge at hour {hour}"
    #                 )
    #
    #         print("✓ DQN no discharge test passed")
    #
    #     except ImportError:
    #         print("⚠ SKIP: DQN no discharge test (TensorFlow not installed)")
    #         self.skipTest("TensorFlow not installed")
    ...
    def test_grid_capacity_constraint(self):
        ...
        for hour, usage in system.grid_usage.items():
            self.assertLessEqual(
                ...
                f"Grid usage {usage:.2f} should not exceed capacity {grid_capacity[hour]:.2f} at hour {hour}"
            )
        
        print("✓ Grid capacity constraint test passed")
    ...
    def test_arrival_departure_times(self):
        """Test that EVs only charge during available times"""
        ...
        for i in range(len(self.evs)):
            self.assertAlmostEqual(
                ...
                initial_socs[i],
                places=6,
                msg=f"EV {i} should not charge before arrival"
            )
        ...
        for i in range(len(self.evs)):
            self.assertAlmostEqual(
                ...
                initial_socs[i],
                places=6,
                msg=f"EV {i} should not charge after departure"
            )
        
        print("✓ Arrival/departure times test passed")
    ...
    def test_building_surplus_utilization(self):
        """Test that building surplus energy is used before grid"""
        print("\n=== Testing Building Surplus Utilization ===")

        system = MultiEVChargingSystem(self.building, self.evs, self.grid, self.grid_capacity)

        # Hour 12 should have solar surplus
        building_surplus = max(0, -self.building.get_net_energy_demand(12))
        
        if building_surplus > 0:
            initial_grid_usage = system.grid_usage[12]
            system.simple_charge_multi(12)
            grid_used = system.grid_usage[12] - initial_grid_usage
            
            # Grid usage should be less than total energy charged if surplus exists
            print(f"Building surplus: {building_surplus:.2f} kWh, Grid used: {grid_used:.2f} kWh")
            print("✓ Building surplus utilization test passed")
        else:
            print("⚠ No building surplus at hour 12, skipping specific test")
    ...
    def test_cost_optimization(self):
        """Test that algorithms minimize charging costs"""
        print("\n=== Testing Cost Optimization ===")

        # Create two scenarios: charge at peak vs off-peak
        peak_cost_total = 0
        offpeak_cost_total = 0

        # Scenario 1: Charge during peak hours
        system1 = MultiEVChargingSystem(self.building, self.evs, self.grid, self.grid_capacity)
        for hour in [17, 18, 19]:  # Peak hours
            _, cost = system1.simple_charge_multi(hour)
            peak_cost_total -= cost

        # Reset EVs for scenario 2
        for ev_cfg in self.evs:
            ev_cfg['ev'].soc = 0.3  # Reset to initial

        # Scenario 2: Charge during off-peak hours
        system2 = MultiEVChargingSystem(self.building, self.evs, self.grid, self.grid_capacity)
        for hour in [9, 10, 11]:  # Off-peak/mid-peak hours
            _, cost = system2.simple_charge_multi(hour)
            offpeak_cost_total -= cost

        print(f"Peak hours cost: {peak_cost_total:.2f}€")
        print(f"Off-peak hours cost: {offpeak_cost_total:.2f}€")
            ...
            try:
                ...
                for ev_cfg in self.evs:
                    if hasattr(ev_cfg['ev'], '_initial_soc'):
                        ...
                        # Store initial SOC for future resets
                ...
                for hour in range(10, 16):  # Test window
