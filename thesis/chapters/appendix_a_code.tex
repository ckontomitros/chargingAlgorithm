...
This appendix provides key code excerpts from the simulation implementation. The complete source code is submitted with the thesis as supplementary material.
...
The complete implementation (Python modules, configuration files, data inputs, and analysis scripts) accompanies this thesis. Usage instructions are documented in \texttt{README\_THESIS.md} at the project root. The main simulation entry points are \texttt{scripts/run\_simulation.py} (charge-only) and the V2G variant under \texttt{src/simulation/}.
...
\begin{enumerate}
    ...
    \item Ensure data files are present under \texttt{data/} (\texttt{office\_single\_building\_timeseries.csv}, \texttt{irradiance\_pvgis.csv}, and YAML configs)
    \item Run: \texttt{python scripts/run\_simulation.py}
    \item Figures and extracted results are written under \texttt{scripts/} and \texttt{thesis/figures/}
\end{enumerate}
