...
March 12, 2018 was selected because it is a spring day with mid-range PV output and variable cloud cover in the Paris PVGIS record. It avoids extreme summer or winter days, and the SARAH3 data for that date are well validated.
...
The code is organized around core models for the building, electric vehicles, and the grid. Charge-only logic lives in \texttt{MultiEVChargingSystem}, while V2G logic is handled by \texttt{MultiEVV2GChargingSystem}. The \texttt{multi\_ev\_simulator} and \texttt{multi\_ev\_v2g\_simulator} scripts run the charge-only and V2G scenarios.
